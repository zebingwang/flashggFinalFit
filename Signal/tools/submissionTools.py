  ...
  if _opts['batch'] == "condor":
    ...
    if( _opts['mode'] == "signalFit" )&( not _opts['groupSignalFitJobsByCat'] ):
      for pidx in range(_opts['nProcs']):
        for cidx in range(_opts['nCats']):
          ...
          if ( _opts['analysis'] == "HHWWgg" or _opts['HHWWggLabel'] == "single_Higgs" ) :
            _f.write("  python %s/scripts/signalFit.py --inputWSDir %s --ext %s --proc %s --cat %s  --HHWWggLabel %s --year %s --analysis %s --massPoints %s --skipVertexScenarioSplit True --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],p,c,_opts['HHWWggLabel'],_opts['year'],_opts['analysis'],_opts['massPoints'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
          else:
            _f.write("  python %s/scripts/signalFit.py --inputWSDir %s --ext %s --proc %s --cat %s  --HHWWggLabel %s --year %s --analysis %s --massPoints %s --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],p,c,_opts['HHWWggLabel'],_opts['year'],_opts['analysis'],_opts['massPoints'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
          _f.write("fi\n")
    ...
    # For looping over categories
      ...
      for cidx in range(_opts['nCats']):
        ...
        _f.write("if [ $1 -eq %g ]; then\n"%cidx)
        if ( _opts['analysis'] == "HHWWgg" or _opts['HHWWggLabel'] == "single_Higgs" ) :
            _f.write("  python %s/scripts/calcPhotonSyst.py --analysis %s --cat %s --procs %s --HHWWggLabel %s --ext %s --inputWSDir %s --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,_opts['analysis'],c,_opts['procs'],_opts['HHWWggLabel'],_opts['ext'],_opts['inputWSDir'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
        else:
            _f.write("  python %s/scripts/calcPhotonSyst.py --cat %s --procs %s --HHWWggLabel %s --ext %s --inputWSDir %s --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,c,_opts['procs'],_opts['HHWWggLabel'],_opts['ext'],_opts['inputWSDir'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
        _f.write("fi\n")
      ...
      for cidx in range(_opts['nCats']):
        ...
        _f.write("if [ $1 -eq %g ]; then\n"%cidx)
        if ( _opts['analysis'] == "HHWWgg" or _opts['HHWWggLabel'] == "single_Higgs" ) :
          _f.write("  python %s/scripts/fTest.py --analysis %s --cat %s --procs %s  --HHWWggLabel %s --ext %s --inputWSDir %s %s\n"%(swd__,_opts['analysis'],c,_opts['procs'],_opts['HHWWggLabel'],_opts['ext'],_opts['inputWSDir'],_opts['modeOpts']))
        else:
          _f.write("  python %s/scripts/fTest.py --cat %s --procs %s  --HHWWggLabel %s --ext %s --inputWSDir %s %s\n"%(swd__,c,_opts['procs'],_opts['HHWWggLabel'],_opts['ext'],_opts['inputWSDir'],_opts['modeOpts']))
        _f.write("fi\n")
    ...
    # For single script
      ...
      _f.write("python %s/scripts/getEffAcc.py --inputWSDir %s --ext %s --procs %s --HHWWggLabel %s --massPoints %s %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],_opts['HHWWggLabel'],_opts['procs'],_opts['massPoints'],_opts['modeOpts']))
    elif _opts['mode'] == 'getDiagProc':
      _f.write("python %s/scripts/getDiagProc.py --inputWSDir %s --ext %s  --HHWWggLabel %s %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],_opts['HHWWggLabel'],_opts['modeOpts']))
    ...
    if( _opts['mode'] == "signalFit" )&( not _opts['groupSignalFitJobsByCat'] ):
      for pidx in range(_opts['nProcs']):
        for cidx in range(_opts['nCats']):
          ...
          writePreamble(_f)
          if ( _opts['analysis'] == "HHWWgg" or _opts['HHWWggLabel'] == "single_Higgs" ) :
              _f.write("python %s/scripts/signalFit.py --inputWSDir %s --ext %s --proc %s --cat %s  --HHWWggLabel %s --year %s --analysis %s --massPoints %s --skipVertexScenarioSplit True --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],p,c,_opts['HHWWgg'],_opts['year'],_opts['analysis'],_opts['massPoints'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
          else:
              _f.write("python %s/scripts/signalFit.py --inputWSDir %s --ext %s --proc %s --cat %s  --HHWWggLabel %s --year %s --analysis %s --massPoints %s --scales \'%s\' --scalesCorr \'%s\' --scalesGlobal \'%s\' --smears \'%s\' %s\n"%(swd__,_opts['inputWSDir'],_opts['ext'],p,c,_opts['HHWWgg'],_opts['year'],_opts['analysis'],_opts['massPoints'],_opts['scales'],_opts['scalesCorr'],_opts['scalesGlobal'],_opts['smears'],_opts['modeOpts']))
          _f.close()
