...
int main(int argc, char *argv[]){
  ...
  //float rangeLow = 115;
  //float rangeHigh = 135;
	float rangeLow = mggblindlow_;//bing
	float rangeHigh = mggblindhigh_;//bing

  //want binning of \0.5GeV
  //int   nBins    = 2* int(rangeHigh -rangeLow);
  //int   nBins    = 2* int(rangeHigh -rangeLow);
	int   nBins    = 8* int(rangeHigh -rangeLow);//bing
  ...
  //  = new WSTFileWrapper(filename_,"tagsDumper/cms_hgg_13TeV");
	WSTFileWrapper *inWS
    = new WSTFileWrapper(filename_,"CMS_hza_workspace");//bing
...
//   if(verbose_) std::cout << "[INFO] Printing all workspace variables:" << inWS->allVars()->writeToFile() << endl;
	...
	RooRealVar *mass = (RooRealVar*)inWS->var("CMS_hza_mass");//bing
    ...
    // now main loop through processes...
      ...
      // access dataset and immediately reduce it!
          ...
          //Form("%s_%s_13TeV_%s",proc.c_str(),HHWWgg_Label.c_str(),flashggCats_[cat].c_str())); // HHWWgg
				RooDataSet *data0   = (RooDataSet*)inWS->data(
          Form("%s_%d_13TeV_%s",proc.c_str(),mass_,flashggCats_[cat].c_str()));//bing
        ...
        //dataWV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass, *dZ));
				...
				data = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass));//bing
				dataRV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass));//bing
        dataWV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass));//bing
        ...
        for (unsigned int i=0 ; i < data0->numEntries() ; i++){
            ...
            //dZ->setVal(data0->get(i)->getRealValue("dZ"));
						mass->setVal(data0->get(i)->getRealValue("CMS_hza_mass"));//bing
						weight0->setVal(data0->get(i)->getRealValue("CMS_hza_weight") );//bing
						data->add( RooArgList(*mass, *weight0), weight0->getVal() );//bing
			...
			*/
			if (1.0){//bing
			dataRV->add( RooArgList(*mass, *weight0), weight0->getVal() );//bing
			} else{
			dataWV->add( RooArgList(*mass, *weight0), weight0->getVal() );//bing
			}
        }
