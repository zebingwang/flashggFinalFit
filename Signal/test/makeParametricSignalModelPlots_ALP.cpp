...
string datfilename_;
int m_hyp_;
float mgglow_ =0.8;//bing
float mgghigh_ =40;//bing
string procString_;
int ncats_;
int binning_;
...
void OptionParser(int argc, char *argv[]){
  ...
  desc1.add_options()
    ...
    ("mhLow,L", po::value<float>(&mgglow_)->default_value(0.8),                                  			"Low ALP mass point")//bing
    ("mhHigh,H", po::value<float>(&mgghigh_)->default_value(40),                                			"High ALP mass point")//bing
    ("sqrts", po::value<int>(&sqrts_)->default_value(13),                                    "CoM energy")
    ("binning", po::value<int>(&binning_)->default_value(80),                                    "CoM energy")
  ...
  desc2.add_options()
    ("ncats,n", po::value<int>(&ncats_)->default_value(1),                                      "Number of cats (Set Automatically if using --isFlashgg 1)")
    ;
...
map<string,RooDataSet*> getFlashggData(RooWorkspace *work, int ncats, int m_hyp){
  ...
  for (int cat=0; cat<ncats; cat++){
    ...
    result.insert(pair<string,RooDataSet*>(Form("%s",flashggCats_[cat].c_str()),(RooDataSet*)work->data(Form("sig_mass_m%2d_%s",m_hyp,flashggCats_[cat].c_str()))));//bing
  }
  //result.insert(pair<string,RooDataSet*>("all",(RooDataSet*)work->data(Form("sig_mass_m%3d_AllCats",m_hyp))));
  result.insert(pair<string,RooDataSet*>("all",(RooDataSet*)work->data(Form("sig_mass_m%2d_AllCats",m_hyp))));//bing
...
map<string,RooDataSet*> getFlashggDataGranular(RooWorkspace *work, int ncats, int m_hyp){
  ...
  for (int cat=0; cat<ncats; cat++){
    ...
    if (verbose_) std::cout << "INFO looking for this workspace: " << Form("sig_%s_mass_m%d_%s",procs_[proc].c_str(),m_hyp,flashggCats_[cat].c_str()) << std::endl;//bing
    std::cerr << "INFO looking for this workspace: " << Form("sig_%s_mass_m%d_%s",procs_[proc].c_str(),m_hyp,flashggCats_[cat].c_str()) << std::endl;//bing
    result.insert(pair<string,RooDataSet*>(Form("%s_%s",procs_[proc].c_str(),flashggCats_[cat].c_str()),(RooDataSet*)work->data(Form("sig_%s_mass_m%d_%s",procs_[proc].c_str(),m_hyp,flashggCats_[cat].c_str()))));//bing
     assert(work->data(Form("sig_%s_mass_m%d_%s",procs_[proc].c_str(),m_hyp,flashggCats_[cat].c_str())));//bing
    }
  }
...
void Plot(RooRealVar *mass, RooDataSet *data, RooAbsPdf *pdf, pair<double,double> sigRange, vector<double> fwhmRange, string title, string savename){
  ...
  while((datavar=(RooAbsReal*)vIter->Next())) {
    ...
    if (TString(datavar->GetName()).Contains("wv")) continue;
    if (TString(datavar->GetName()).Contains("rvFracFunc")) continue;
...
int main(int argc, char *argv[]){
  ...
  mass->setRange("higgsRange",mgglow_,mgghigh_);//bing
  ...
  for (map<string,RooDataSet*>::iterator dataIt=dataSets.begin(); dataIt!=dataSets.end(); dataIt++){
    ...
    if(dataIt->first!="all") thisSigRange = getEffSigma(mass,pdfs[dataIt->first],mgglow_,mgghigh_);//bing
    else {
      //thisSigRange = getEffSigBinned(mass,pdfs[dataIt->first],m_hyp_-10.,m_hyp_+10);
      thisSigRange = getEffSigBinned(mass,pdfs[dataIt->first],mgglow_,mgghigh_);//bing
    ...
    vector<double> thisFWHMRange = getFWHM(mass,pdfs[dataIt->first],dataIt->second,mgglow_,mgghigh_);//bing
    ...
    if (doCrossCheck_) performClosure(mass,pdfs[dataIt->first],dataIt->second,Form("%s/closure_%s.pdf",outfilename_.c_str(),dataIt->first.c_str()),mgglow_,mgghigh_,thisSigRange.first,thisSigRange.second);//bing
  ...
  for (map<string,RooDataSet*>::iterator dataIt=dataSetsGranular.begin(); dataIt!=dataSetsGranular.end(); dataIt++){
    ...
    pair<double,double> thisSigRange = getEffSigma(mass,pdfsGranular[dataIt->first],mgglow_,mgghigh_);//bing
    ...
    vector<double> thisFWHMRange = getFWHM(mass,pdfsGranular[dataIt->first],dataIt->second,mgglow_,mgghigh_);//bing
    sigEffs.insert(pair<string,double>(dataIt->first,(thisSigRange.second-thisSigRange.first)/2.));
    fwhms.insert(pair<string,double>(dataIt->first,thisFWHMRange[1]-thisFWHMRange[0]));
    ...
    if (doCrossCheck_) performClosure(mass,pdfsGranular[dataIt->first],dataIt->second,Form("%s/closure_%s.pdf",outfilename_.c_str(),dataIt->first.c_str()),mgglow_,mgghigh_,thisSigRange.first,thisSigRange.second);//bing
