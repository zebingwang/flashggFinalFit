...
RooDataSet * reduceDataset(RooDataSet *data0){
  ...
  //RooDataSet *data = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_, *dZ_));
	RooDataSet *data = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_));//bing
...
RooDataSet * rvwvDataset(RooDataSet *data0, string rvwv){
  ...
  //RooDataSet *dataWV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_, *dZ_));
	RooDataSet *dataRV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_));//bing
  RooDataSet *dataWV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_));//bing
	RooRealVar *weight0 = new RooRealVar("weight","weight",-100000,1000000);
  for (unsigned int i=0 ; i < data0->numEntries() ; i++){
    //mass_->setVal(data0->get(i)->getRealValue("CMS_hgg_mass"));
    ...
    // }
		//bing(start)
    if (1){
      dataRV->add( RooArgList(*mass_, *weight0), weight0->getVal() );
    } else{
      dataWV->add( RooArgList(*mass_, *weight0), weight0->getVal() );
    }//bing(end)
  }
  ...
  for (unsigned int iLine = 0 ; iLine < map_proc_.size() ; iLine++){
    ...
    for (int mhIndex=0; mhIndex< massList_.size() ; mhIndex++){
      ...
      RooDataHist *dataH;
        ...
        //RooDataSet *data0   = reduceDataset((RooDataSet*)inWS->data(Form("%s_%s_13TeV_%s_%d",proc.c_str(),HHWWgg_Label.c_str(),cat.c_str(),mh)));
				RooDataSet *data0   = reduceDataset((RooDataSet*)inWS->data(Form("%s_%d_13TeV_%s",proc.c_str(),mh,cat.c_str())));//bing
	...
	//plotBeamSpotDZdist(data,"VHdebug");
        ...
        //if (nEntriesRV < 200 || sEntriesRV < 0 || ( userSkipRV)){
				if (nEntriesRV < 20 || sEntriesRV < 0 || ( userSkipRV)){//bing
                        		...
                        		intLumiReweigh(
                          		reduceDataset(
                          			...
                          			//(RooDataSet*)inWS->data(Form("%s_%s_13TeV_%s_%d",replancementProc.c_str(),HHWWgg_Label.c_str(),replancementCat.c_str(),mh)) // HHWWgg
																(RooDataSet*)inWS->data(Form("%s_%d_13TeV_%s",replancementProc.c_str(),mh,replancementCat.c_str()))//bing
...
cout << "[INFO] Starting to combine2 fits..." << endl;
...
cout << "[INFO] Starting to combine3 fits..." << endl;
    // if we are doing jobs for each proc/tag, want to do the split.
    bool split =0;
    if (split_.size() > 0) split=1;

    string Procs ="ggF";
    string Tag= "HHWWggTag_0";
    packager.packageOutput(/*split*/split, Procs,Tag);
    sw.Stop();
