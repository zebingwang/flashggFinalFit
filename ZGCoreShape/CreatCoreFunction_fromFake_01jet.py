...
if os.path.exists(Corefile_path):
    ...
    smooth_bins = {"cat0": 400,
                    "cat1": 250,
                    "cat2": 250,
                    "cat3": 100,
                    "allcat": 200}
    smooth_iteration = {"cat0": 200000,
                    "cat1": 5000,
                    "cat2": 5000,
                    "cat3": 2000,
                    "allcat": 10000}
    ...
    #for cat in ["cat3"]:
        ...
        print("[INFO]: smooting cat: {}".format(cat))
        if KernelSmooth:
            keysPdfs["MC_{}".format(cat)] = rt.RooKeysPdf("CoreShape_{}".format("MC_{}".format(cat)), "CoreShape_{}".format("MC_{}".format(cat)), H_mass, MC["MC_{}".format(cat)], rt.RooKeysPdf.MirrorBoth, smooth_rhos[cat])
        else:
            print("[INFO]: KernelSmooth = False")
            hist = rt.TH1F("hist_{}".format(cat), "Histogram from RooDataSet", smooth_bins[cat], H_mass.getMin(), H_mass.getMax())
            MC["MC_{}".format(cat)].fillHistogram(hist, rt.RooArgList(H_mass))
            hist.Smooth(smooth_iteration[cat], "k5a")
            hist_data = rt.RooDataHist("hist_data_{}".format(cat), "Histogram Data", rt.RooArgList(H_mass), hist)
            keysPdfs["MC_{}".format(cat)] = rt.RooHistPdf("CoreShape_{}".format("MC_{}".format(cat)), "CoreShape_{}".format("MC_{}".format(cat)), rt.RooArgSet(H_mass), hist_data)

            #keysPdfs["MC_{}".format(cat)].Print()
        
        getattr(w,'import')(keysPdfs["MC_{}".format(cat)])
        #keysPdfs["MC_{}".format(cat)].Print()

        keysPdfs_plots["MC_{}".format(cat)] = smooth_plot(keysPdfs["MC_{}".format(cat)], H_mass, MC["MC_{}".format(cat)], data["data_{}".format(cat)])
        keysPdfs_plots["MC_{}".format(cat)].SaveAs('./{}_fromFake.png'.format(cat))
