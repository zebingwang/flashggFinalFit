...
do
  ...
  do
    ...
    cat='HHWWggTag_FH_0' #Final cat name 
    NewTreeCats='"HHWWggTag_FH_0"' #c++ vector style 
    catNames=("HHWWggTag_FH_0") #shell format array, should same as "cat",just 8different format
    TreeCats='"HHWWggTag_1"' #input cat name in the Signal tree
    DataTreeCats='"HHWWggTag_1"' #input cat name in the Data tree
    NewTreeCats='"HHWWggTag_FH_0"' #output tree cat name.
    TreeFile="/afs/cern.ch/user/r/rasharma/work/public/doubleHiggs/ntuples/GluGluToHHTo2G2ZTo2G4Q_node_cHHH1_2016.root"
    DataTreeFile="/afs/cern.ch/user/r/rasharma/work/public/doubleHiggs/ntuples/allData.root"
    Replace='HHWWggTag_FH_0'
    doSelections="0"
...
############################################
  ...
  sed -i "s#NEW_Cat_NAME#${NewTreeCats}#g" Selections_Run.C 
  ...
  sed -i "s#INPUTFILE#${TreeFile}#g" Selections_Run.C
if [ "$ext" = "SL" ]
then
...
##########Data selection #####
  sed -i "s#CAT#${DataTreeCats}#g" DataSelections_Run.C
  sed -i "s#NEW_Cat_NAME#${NewTreeCats}#g" DataSelections_Run.C 
...
###################
python RunYields.py --cats ${cat} --inputWSDirMap ${year}=${path}/Signal/Input --procs ${procs},tth,wzh --doSystematics --doHHWWgg True --HHWWggLabel node_${node} --batch local --ext SingleHiggs  --bkgModelWSDir ./Models --sigModelWSDir ./Models
####################
...
sed -i "22c process  0 2 3  1" ./HHWWgg_${procs}_node_${node}_${cat}_${year}.txt
