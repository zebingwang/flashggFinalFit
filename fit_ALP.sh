...
./bin/makeBkgPlots_ALP -b $path_out_bkg/CMS-HGG_mva_13TeV_multipdf.root -d $path_out_bkg/BkgPlots -o $path_out_bkg/BkgPlots.root -S 13 --isMultiPdf --useBinnedData  --doBands --massStep 1 --mhVal 15 --mhLow 0 --mhHigh 40 --intLumi 41.5 -c 0 --isFlashgg 0 --mhLowBlind 5 --mhHighBlind 30
...
for ((iBin=0; iBin<$nMass; iBin++))
    ...
    ./bin/signalFTest_ALP -i $path_in_sig/ALP_data_sig_workspace_M${massList[$iBin]}.root -d $path_out_bkg/ALPmassInde_data_sig.dat -o $path_out_bkg/HZAmassInde_ftest -p data -f cat0 -m ${massList[$iBin]} --mhLowBlind ${massList_dn[$iBin]} --mhHighBlind ${massList_up[$iBin]}

    sed -i "s/data/ggh/" $path_out_bkg/ALPmassInde_data_sig.dat
    #sed -i "s/$/ 1/g" $path_out_bkg/ALPmassInde_data_sig.dat

    ./bin/SignalFit_ALP -i $path_in_sig/ALP_data_sig_m${massList_dn[$iBin]}_workspace.root,$path_in_sig/ALP_data_sig_m${massList[$iBin]}_workspace.root,$path_in_sig/ALP_data_sig_m${massList_up[$iBin]}_workspace.root -o $path_out_bkg/CMS-HGG_sigfit_data_ggh_cat0.root -p $path_out_bkg/plots_ALP --procs ggh -f cat0 -d $path_out_bkg/ALPmassInde_data_sig.dat -s empty.dat --nBins ${nBins[$iBin]} --changeIntLumi 41.5 --useSSF 1 -L ${mhLowBlind[$iBin]} --mhHigh ${mhHighBlind[$iBin]} --massList ${massList_dn[$iBin]},${massList[$iBin]},${massList_up[$iBin]}
    ...
    python makeParametricModelDatacardFLASHgg_ALP.py -i $path_out_bkg/CMS-HGG_sigfit_data_ggh_cat0.root -o $path_out_bkg/datacard_ALPmass${massList[$iBin]}.txt -p ggh -c cat0 --photonCatScales empty.dat --isMultiPdf --intLumi 41.5 -m ${massList[$iBin]}
