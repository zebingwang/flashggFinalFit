...
def addConstantSyst(sd, _syst, options):
    ...
    if _syst['correlateAcrossYears'] == 1:
        sd[_syst['name']] = '-'
        if fromJson:
            sd.loc[(sd['type'] == 'sig'), _syst['name']] = sd[(sd['type'] == 'sig')].apply(
                lambda x: getValueFromJson(x, uval, _syst['name']), axis=1)
        else:
            # If signal and not NOTAG then set value
            sd.loc[(sd['type'] == 'sig') & (~sd['cat'].str.contains(
                "NOTAG")), _syst['name']] = _syst['value']
    ...
    # Partial correlation
        ...
        for year in options.years.split(","):
            mask = (sd['type'] == 'sig') & (
                ~sd['cat'].str.contains("NOTAG")) & (sd['year'] == year)
    ...
    # If not correlate across years then create separate columns for each year and fill separately
        ...
        for year in options.years.split(","):
            ...
            sd.loc[(sd['type'] == 'sig') & (sd['year'] == year) & (~sd['cat'].str.contains(
                "NOTAG")), "%s_%s" % (_syst['name'], year)] = _syst['value'][year]
...
def factoryType(d, s):
    ...
    if('pdfWeight' in s['name']):
        return "s_w"
    if('scaleWeight' in s['name']):
        return "s_w"
    if('alphaSWeight' in s['name']):
        return "s_w"
...
def calcSystYields(_nominalDataName, _nominalDataContents, _inputWS, _systFactoryTypes, skipCOWCorr=True, proc="ggH", year='2016', ignoreWarnings=False):
    ...
    for s, f in _systFactoryTypes.iteritems():
        if f in ["a_h", "a_w"]:
            for direction in ['up', 'down']:
                ...
                if not skipCOWCorr:
                    systYields["%s_%s_COWCorr" % (s, direction)] = 0
        else:
            systYields[s] = 0
            if not skipCOWCorr:
                systYields["%s_COWCorr" % s] = 0
    ...
    for i in range(0, data_nominal.numEntries()):
        ...
        #  print "Event:",i
        f_COWCorr = p.getRealValue(
            "centralObjectWeight") if "centralObjectWeight" in _nominalDataContents else 1.
        f_NNLOPS = abs(p.getRealValue("NNLOPSweight")
                       ) if "NNLOPSweight" in _nominalDataContents else 1.
        ...
        for s, f in _systFactoryTypes.iteritems():
            ...
            # If asymmetric weights:
                ...
                if s in systToSkip:
                    ...
                    if not skipCOWCorr:
                        if f_COWCorr != 0:
                            systYields["%s_up_COWCorr" %
                                       s] += w*(f_NNLOPS/f_COWCorr)
                            systYields["%s_down_COWCorr" %
                                       s] += w*(f_NNLOPS/f_COWCorr)

                else:
                    centralWeightStr = "centralObjectWeight"
                    f_central = p.getRealValue(
                        centralWeightStr) if centralWeightStr in _nominalDataContents else 1.
                    f_up, f_down = p.getRealValue(
                        "%sUp01sigma" % s), p.getRealValue("%sDown01sigma" % s)
                    import numpy as np
                    ...
                    if not skipCOWCorr:
                        if f_COWCorr != 0:
                            systYields["%s_up_COWCorr" %
                                       s] += w_up*(f_NNLOPS/f_COWCorr)
                            systYields["%s_down_COWCorr" %
                                       s] += w_down*(f_NNLOPS/f_COWCorr)
            ...
            # If symmetric weights
                ...
                if s in systToSkip:
                    ...
                    if not skipCOWCorr:
                        if f_COWCorr != 0:
                            systYields["%s_COWCorr" %
                                       s] += w*(f_NNLOPS/f_COWCorr)

                else:
                    if "scaleWeight" in s:
                        centralWeightStr = "scaleWeights[0]"
                    elif "alphaSWeight" in s:
                        centralWeightStr = "scaleWeights[0]"
                    elif "pdfWeight" in s:
                        centralWeightStr = "pdfWeights[0]"
                    else:
                        centralWeightStr = "centralObjectWeight"
                    f_central = p.getRealValue(
                        centralWeightStr) if centralWeightStr in _nominalDataContents else 1.
                    ...
                    # 2) only central weight is zero then skip event
                        ...
                        if not skipCOWCorr:
                            if f_COWCorr != 0:
                                systYields["%s_COWCorr" % s] += w * \
                                    (f_NNLOPS/f_COWCorr)*(f/f_central)

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # For systematics stored as separate RooDataHists
    for s, f in _systFactoryTypes.iteritems():
        if f == "a_h":
            data_hist_up, data_hist_down = _inputWS.data("%s_%sUp01sigma" % (
                _nominalDataName, s)), _inputWS.data("%s_%sDown01sigma" % (_nominalDataName, s))
...
def experimentalSystFactory(d, systs, ftype, options, _removal=False):
    ...
    for s in systs:
        ...
        if s['correlateAcrossYears']:
            ...
            d.loc[mask, s['name']] = d[mask].apply(
                lambda x: compareYield(x, f, s['name']), axis=1)
        else:
            for year in options.years.split(","):
                mask = (d['type'] == 'sig') & (
                    ~d['cat'].str.contains("NOTAG")) & (d['year'] == year)
                d.loc[mask, '%s_%s' % (s['name'], year)] = d[mask].apply(
                    lambda x: compareYield(x, f, s['name']), axis=1)
        ...
        if _removal:
            if f in ['a_h', 'a_w']:
                for direction in ['up', 'down']:
                    d.drop(['%s_%s_yield' % (s['name'], direction)],
                           axis=1, inplace=True)
            else:
                d.drop(['%s_yield' % s['name']], axis=1, inplace=True)
...
def theorySystFactory(d, systs, ftype, options, stxsMergeScheme=None, _removal=False):
    ...
    for proc_s0 in d[d['type'] == 'sig'].proc_s0.unique():
        for year in options.years.split(","):
            ...
            d.loc[mask, 'proc_s0_nominal_yield'] = d[mask]['nominal_yield%s' %
                                                           corrExt].sum()
            for s in systs:
                if s['type'] == 'constant':
                    continue
                ...
                if f in ['a_w', 'a_h']:
                    for direction in ['up', 'down']:
                        d.loc[mask, 'proc_s0_%s_%s_yield' % (s['name'], direction)] = d[mask]['%s_%s_yield%s' % (
                            s['name'], direction, corrExt)].sum()
                else:
                    d.loc[mask, 'proc_s0_%s_yield' % s['name']
                          ] = d[mask]['%s_yield%s' % (s['name'], corrExt)].sum()
    ...
    for proc in d[d['type'] == 'sig'].proc.unique():
        mask = (d['proc'] == proc)
        d.loc[mask, 'proc_nominal_yield'] = d[mask]['nominal_yield%s' %
                                                    corrExt].sum()
        for s in systs:
            if s['type'] == 'constant':
                continue
            ...
            if f in ['a_w', 'a_h']:
                for direction in ['up', 'down']:
                    d.loc[mask, 'proc_%s_%s_yield' % (s['name'], direction)] = d[mask]['%s_%s_yield%s' % (
                        s['name'], direction, corrExt)].sum()
            else:
                d.loc[mask, 'proc_%s_yield' % s['name']
                      ] = d[mask]['%s_yield%s' % (s['name'], corrExt)].sum()
    ...
    if options.doSTXSMerging:
        for mergeName, mergeBins in stxsMergeScheme.iteritems():
            for year in options.years.split(","):
                ...
                for mb in mergeBins:
                    mBins.append("%s_%s_hgg" % (mb, year))
                mask = (d['type'] == 'sig') & (
                    d.apply(lambda x: x['proc'] in mBins, axis=1))
                d.loc[mask, 'merge_%s_nominal_yield' %
                      mergeName] = d[mask]['nominal_yield%s' % corrExt].sum()
                # Loop over systematics
                for s in systs:
                    if s['type'] == 'constant':
                        continue
                    elif 'mnorm' not in s['tiers']:
                        continue
                    f = ftype[s['name']]
                    if f in ['a_w', 'a_h']:
                        for direction in ['up', 'down']:
                            d.loc[mask, 'merge_%s_%s_%s_yield' % (mergeName, s['name'], direction)] = d[mask]['%s_%s_yield%s' % (
                                s['name'], direction, corrExt)].sum()
                    else:
                        d.loc[mask, 'merge_%s_%s_yield' % (
                            mergeName, s['name'])] = d[mask]['%s_yield%s' % (s['name'], corrExt)].sum()
    ...
    for s in systs:
        ...
        if "THU_ggH" in s['name']:
            mask = (d['type'] == 'sig') & (d['nominal_yield']
                                           != 0) & (d['proc'].str.contains('ggH'))
        else:
        ...
        for tier in s['tiers']:
            if tier == 'mnorm':
                continue
            d.loc[mask, "%s_%s" % (s['name'], tier)] = d[mask].apply(
                lambda x: compareYield(x, f, s['name'], mode=tier), axis=1)
        ...
        for mergeName in stxsMergeScheme:
            for s in systs:
                ...
                for year in options.years.split(","):
                    ...
                    mask = (d['merge_%s_nominal_yield' % mergeName] == d['merge_%s_nominal_yield' % mergeName]) & (
                        d['year'] == year) & (d['nominal_yield'] != 0)
                    d.loc[mask, "%s_%s_mnorm" % (s['name'], mergeName)] = d[mask].apply(
                        lambda x: compareYield(x, f, s['name'], mode='mnorm', mname=mergeName), axis=1)
    ...
    if _removal:
        ...
        if options.doSTXSMerging:
            for mergeName in stxsMergeScheme:
                ids_.append("merge_%s_" % mergeName)
        # Loop over systematics
        for s in systs:
            if s['type'] == 'constant':
                continue
            ...
            if f in ['a_h', 'a_w']:
                for direction in ['up', 'down']:
                    for id_ in ids_:
                        d.drop(['%s%s_%s_yield' % (id_, s['name'],
                                                   direction)], axis=1, inplace=True)
            else:
                for id_ in ids_:
                    d.drop(['%s%s_yield' % (id_, s['name'])],
                           axis=1, inplace=True)

        # Remove also nominal yields for all combinations
        ids_.remove('')
        for id_ in ids_:
            d.drop(['%snominal_yield' % id_], axis=1, inplace=True)
...
def compareYield(row, factoryType, sname, mode='default', mname=None):
    ...
    if mode != 'default':  # only for theory uncertainties...
        ...
        if factoryType in ["a_w", "a_h"]:
            for direction in ['up', 'down']:
                if row["proc_%s_%s_yield" % (sname, direction)] == 0:
                    return [1., 1.]
        else:
            if row["proc_%s_yield" % sname] == 0:
                return [1.]
    ...
    if(mode == 'default') | (mode == 'ishape'):
        ...
        if factoryType == "a_h":
            midpoint_yield = 0.5 * \
                (row["%s_down_yield" % sname]+row["%s_up_yield" % sname])
            if midpoint_yield == 0:
                return [1., 1.]
            else:
                return [(row["%s_down_yield" % sname]/midpoint_yield), (row["%s_up_yield" % sname]/midpoint_yield)]
        elif factoryType == "a_w":
            return [(row["%s_down_yield" % sname]/row['nominal_yield']), (row["%s_up_yield" % sname]/row['nominal_yield'])]
        else:
            return [row["%s_yield" % sname]/row['nominal_yield']]

    elif mode == 'shape':
        if factoryType in ["a_w", "a_h"]:
            shape_up = (row["%s_up_yield" % sname]/row['nominal_yield']) / \
                (row["proc_%s_up_yield" % sname]/row["proc_nominal_yield"])
            shape_down = (row["%s_down_yield" % sname]/row['nominal_yield']) / \
                (row["proc_%s_down_yield" % sname]/row["proc_nominal_yield"])
            return [shape_down, shape_up]
        else:
            shape = (row["%s_yield" % sname]/row['nominal_yield']) / \
                (row["proc_%s_yield" % sname]/row["proc_nominal_yield"])
            return [shape]

    elif mode == 'norm':
        if factoryType in ["a_w", "a_h"]:
            norm_up = (row["proc_%s_up_yield" % sname]/row["proc_nominal_yield"]) / \
                (row["proc_s0_%s_up_yield" % sname]/row["proc_s0_nominal_yield"])
            norm_down = (row["proc_%s_down_yield" % sname]/row["proc_nominal_yield"])/(
                row["proc_s0_%s_down_yield" % sname]/row["proc_s0_nominal_yield"])
            return [norm_down, norm_up]
        else:
            norm = (row["proc_%s_yield" % sname]/row["proc_nominal_yield"]) / \
                (row["proc_s0_%s_yield" % sname]/row["proc_s0_nominal_yield"])
            return [norm]

    elif mode == 'mnorm':
        if factoryType in ["a_w", "a_h"]:
            mnorm_up = (row["proc_%s_up_yield" % sname]/row["proc_nominal_yield"])/(
                row["merge_%s_%s_up_yield" % (mname, sname)]/row["merge_%s_nominal_yield" % mname])
            mnorm_down = (row["proc_%s_down_yield" % sname]/row["proc_nominal_yield"])/(
                row["merge_%s_%s_down_yield" % (mname, sname)]/row["merge_%s_nominal_yield" % mname])
            return [mnorm_down, mnorm_up]
        else:
            mnorm = (row["proc_%s_yield" % sname]/row["proc_nominal_yield"])/(
                row["merge_%s_%s_yield" % (mname, sname)]/row["merge_%s_nominal_yield" % mname])
            return [mnorm]

    elif mode == 'inorm':
        if factoryType in ["a_w", "a_h"]:
            inorm_up = (row["proc_%s_up_yield" %
                            sname]/row["proc_nominal_yield"])
            inorm_down = (row["proc_%s_down_yield" %
                              sname]/row["proc_nominal_yield"])
        ...
        if factoryType in ["a_w", "a_h"]:
            inc_up = (row["proc_s0_%s_up_yield" %
                          sname]/row["proc_s0_nominal_yield"])
            inc_down = (row["proc_s0_%s_down_yield" %
                            sname]/row["proc_s0_nominal_yield"])
            return [inc_down, inc_up]
        else:
            inc = (row["proc_s0_%s_yield" % sname] /
                   row["proc_s0_nominal_yield"])
            return [inc]
...
def groupSystematics(d, systs, options, prefix="scaleWeight", groupings=[], stxsMergeScheme=None, _removal=False):
    ...
    for group_idx in range(len(groupings)):
            ...
            if tier == 'mnorm':
                if options.doSTXSMerging:
                    ...
                    for mergeName in stxsMergeScheme:
                        ...
                        gr_name = "%s_gr%g_%s_%s" % (
                            prefix, group_idx, mergeName, tier)
                        d[gr_name] = '-'
                        # Define mask as all entries where d[s0_name]!='-'
                        mask = (d[s0_name] != '-') & (d[s1_name] != '-')
                        d.loc[mask, gr_name] = d[mask].apply(
                            lambda x: [x[s0_name][0], x[s1_name][0]], axis=1)
                        # Remove original columns from dataFrame
                        if _removal:
                            for i in gr:
                                d.drop(
                                    ['%s_%g_%s_%s' % (prefix, i, mergeName, tier)], axis=1, inplace=True)
                else:
                    continue
            else:
        ...
        for s in systs:
            ...
            if s['name'] == "%s_%g" % (prefix, gr[0]):
                s['name'] = re.sub("%s_%g" % (prefix, gr[0]),
                                   "%s_gr%g" % (prefix, group_idx), s['name'])
                s['title'] = re.sub("%s_%g" % (prefix, gr[0]), "%s_gr%g" % (
                    prefix, group_idx), s['title'])
            elif s['name'] == "%s_%g" % (prefix, gr[1]):
                systs.remove(s)
        ...
        if tier == 'mnorm':
            if options.doSTXSMerging:
                ...
                for mergeName in stxsMergeScheme:
                    ...
                    mask = (d['%s_%s_%s' %
                              (s_regexp[0]['name'], mergeName, tier)] != '-')
                    d.loc[mask, env_name] = d[mask].apply(
                        lambda x: compareSystForEnvelope(x, s_regexp, tier, mname=mergeName), axis=1)
                    # Remove original columns from dataFrame
                    if _removal:
                        for s in s_regexp:
                            d.drop(
                                ["%s_%s_%s" % (s['name'], mergeName, tier)], axis=1, inplace=True)
            else:
                continue
        else:
            ...
            d.loc[mask, env_name] = d[mask].apply(
                lambda x: compareSystForEnvelope(x, s_regexp, tier), axis=1)
            # Remove original columns from dataFrame
            if _removal:
                for s in s_regexp:
                    d.drop(["%s_%s" % (s['name'], tier)], axis=1, inplace=True)
...
def compareSystForEnvelope(row, systs, stier, mname=None):
    ...
    for sidx in range(len(systs)):
        ...
        if mname is not None:
            sname = "%s_%s_%s" % (s['name'], mname, stier)
        else:
            sname = "%s_%s" % (s['name'], stier)
        if len(row[sname]) == 2:
            e_symm = 0.5*(abs(row[sname][0]-1)+abs(row[sname][1]-1))
        else:
            e_symm = abs(row[sname][0]-1)
        if e_symm > e_symm_max:
            e_symm_max = e_symm
            env_idx = sidx
    env_s = systs[env_idx]
    if mname is not None:
        env_sname = "%s_%s_%s" % (env_s['name'], mname, stier)
    else:
        env_sname = "%s_%s" % (env_s['name'], stier)
