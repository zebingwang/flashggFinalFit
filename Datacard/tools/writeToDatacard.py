    ...
    if s['type'] == 'signal_shape':
        ...
        if s['mode'] in ['scales', 'smears']:
            for year in options.years.split(","):
                ...
                lsyst = "%-70s  param    %-6s %-6s" % (
                    stitle_y, s['mean'], s['sigma'])
                f.write("%s\n" % lsyst)
        else:
            lsyst = "%-70s  param    %-6s %-6s" % (
                stitle, s['mean'], s['sigma'])
            f.write("%s\n" % lsyst)
        return True
                            ...
                            for cat in d.cat.unique():
                                for ir, r in d[d['cat'] == cat].iterrows():
                                    ...
                                    if p in psProcs:
                                        ...
                                        sval = '-'
                                    lsyst = addSyst(
                                        lsyst, sval, stitle, r['proc'], cat)
                            # Remove final space from line and add to file
                            f.write("%s\n" % lsyst[:-1])
            else:
                for year in options.years.split(","):
                    stitle = "%s%s%s_%s" % (
                        s['title'], mergeStr, tierStr, year)
